...
**Update from simulation data**: a later test against TNG300 (see "Validation against a cosmological simulation" below), using true rather than measured mass, found this trend mostly disappears when richness is held fixed across the mass range -- the strong monotonic pattern seen against Tempel/CIRS references was likely driven substantially by those references' own mass-dependent systematics, not a real property of the caustic technique. A follow-up test extending to poorer, genuinely lower-mass halos (where richness was *not* held fixed, and instead varied naturally with mass as it does in any real halo population) found the original richness-driven pattern reappears cleanly: `fbr≈0.42` at median richness ≈2300, `≈0.47-0.48` at N≈200, `≈0.60-0.61` at N≈60-100. The full picture: `fbr`'s real driver is richness, not mass directly; the two are correlated in real cluster samples (rich clusters tend to be massive), which is why the original Tempel/CIRS trend looked mass-dependent. The specific correction shape found against those real-data references (roughly double `fbr` for the most massive clusters) should be read as a richness effect in mass-dependent clothing, not a property of mass or the caustic technique per se.
...
**This isn't a novel idea -- `fbr`/F_β(r) has been calibrated against simulations since the method's introduction, and the history is directly relevant to how to read the number below.** Diaferio (1999) calibrated against mock catalogs using halos (10+ particles) as galaxy tracers and found ⟨F_β(r)⟩=0.5; Serra et al. (2011) used dark matter particles directly instead and found ⟨F_β(r)⟩=0.7 -- a ≈30% systematic mass difference between the two, attributed specifically to which tracer stands in for "galaxy" in the mock (sub-halos are known to be biased tracers of the dark matter velocity dispersion; see Gifford, Maller & Katz 2013, "A Systematic Analysis of Caustic Methods for Galaxy Cluster Masses," who found ⟨F_β(r)⟩=0.65 with a semi-analytic galaxy population and ⟨F_β(r)⟩=0.5 -- matching Diaferio's halo-based value exactly -- when re-calibrated on Millennium Simulation sub-halos alone). More directly relevant still: Pizzardo et al. (2023, "An IllustrisTNG View of the Caustic Technique for Galaxy Cluster Mass Estimation") used this exact simulation, TNG300, for this exact purpose -- the first F_β calibration using actual simulated galaxies (not halos or raw dark matter particles) as tracers, also examining its redshift dependence out to z≈1. The `dx/dy/dz` data used here are subhalo positions (per the accompanying data description), closer in spirit to Diaferio's halo-based tracers than to Pizzardo's stellar/luminous galaxy population or Serra's raw dark matter -- a plausible reason our own value (below) doesn't land exactly on any of theirs.

Also worth carrying forward: Serra et al. (2011) note that F_β is *not* constant with radius in their simulations, and that caustic mass profiles can over-estimate mass specifically within ≈0.3×R200, where velocity anisotropy weakens. This is suggestively close to the `R200/3` inner boundary already used for the NFW-fit radial range in this code (see `fitting_radii` in the earlier `fit_r_upper_mult`/`fit_r_lower_div` discussion) and to the still-unexplained "falling near centre" escape-curve observation earlier in this document -- not proof of a connection, but a concrete literature-backed hypothesis worth checking first if that observation is revisited.

| Source | Tracer used | ⟨F_β(r)⟩ |
|---|---|---|
| Diaferio (1999) | Halos (10+ particles) | 0.5 |
| Serra et al. (2011) | Dark matter particles | 0.7 |
| Gifford et al. (2013), semi-analytic | Semi-analytic galaxies | 0.65 |
| Gifford et al. (2013), re-calibrated | Millennium sub-halos | 0.5 |
| **This project, TNG300, full richness** | **Subhalos** | **0.42-0.43** |
| **This project, TNG300, N≈200** | **Subhalos** | **0.47-0.48** |

Our value sits below the historical range (0.5-0.7) rather than inside it. Given the pattern above -- richer/more numerous, lower-velocity-dispersion tracers pull F_β down -- this isn't obviously wrong, but it hasn't been reconciled with the literature values either; treat it as this specific pipeline's own calibration (already cross-checked against CIRS above) rather than a challenge to the published numbers, which used different simulations, different tracer definitions, and in some cases different radial ranges.
...
**The mass-`fbr` dependence found on Tempel/CIRS mostly disappears against true mass.** Splitting into mass terciles the same way as before: optimal `fbr` came out to ≈0.46 (low), ≈0.40 (mid), ≈0.41 (high) -- flat, not the strong monotonic trend (≈0.35→0.60) found against Tempel/CIRS references. This is a significant update to the earlier finding: it now looks like most or all of that mass-dependent trend was an artifact of the *reference* masses' own mass-dependent systematics (Tempel's virial masses, CIRS/Sohn's caustic masses), not a real property of the caustic technique itself, exactly the ambiguity flagged as unresolved in the `fbr` section above. **Update**: this tercile test used a fixed downsampled richness (N≈200) for every cluster, which -- as later confirmed directly (see "Extending to poorer, lower-mass halos" below) -- incidentally controlled for richness at the same time it varied mass, which is exactly why no trend showed up here. The full explanation turned out to be that the original real-data trend was a richness effect, not that mass doesn't matter at all when richness is allowed to vary naturally with it.
...
### Re-attempting the usual parameter sweep against TNG300 (realistic richness, N≈200)

With genuine ground truth available, it's natural to ask whether the parameters validated against real data (`blur_gaussian`, `gradu`, `q`, `mirror`) are still the best choice here, or whether this cleaner reference suggests something different. Tested at N≈200 (downsampled from full richness, to be representative of a real survey rather than the simulation's full member count):

- **`blur_gaussian`**: Deriche (`FALSE`, the default) still edges out Young-van Vliet, consistent with the real-data finding, though the margin is small here.
- **`gradu`**: no detectable effect at all across 0.5-2.0 -- identical results to the digit. The regime where `gradu` matters on real data (noisier, less complete samples) doesn't seem to bind here.
- **`q`**: the default (`q=10`) was the best of everything tested (4-20) -- moving either direction made things worse. A useful confirmation that the default isn't badly wrong for this kind of wide-window extraction.
- **`fbr` needed re-calibrating for this richness specifically**: 0.42 (calibrated at full simulation richness) was too low at N≈200, giving a real -5.5% to -6.2% bias. Re-sweeping at N≈200 found the zero-bias point closer to `fbr≈0.47-0.48` -- expected and consistent with the richness-precision relationship documented elsewhere in this file (lower richness needs a somewhat higher `fbr` to compensate).
- **`mirror=FALSE`, combined with a further-adjusted `fbr≈0.52`, outperformed the best `mirror=TRUE` configuration on this sample**: M200 bias≈0 with sd=0.114, vs. the best `mirror=TRUE` combination's sd=0.129 (≈13% worse). This is a genuine, reproducible result *on this specific sample* (43 TNG300 halos, this extraction convention, this richness) -- but given the immediately preceding finding that `mirror=FALSE`'s benefit is viewing-angle-dependent even for individual unrelaxed clusters, and that `mirror=TRUE` remains clearly better across all three large real-data samples (Tempel Constrained/Unconstrained, CIRS), this is **not** adopted as a default change. It's recorded as a data point suggesting `mirror`'s general-purpose default might be worth another look with a larger, real-data sample someday, not as evidence the current default is wrong.

**Informed mode was also tested against this simulation (N≈200, true R200 and a true velocity dispersion computed directly from the members within it)**: bias at `fbr=0.42` was essentially unchanged from blind mode (-0.054 vs. -0.055 dex), but scatter improved by ≈19% (sd=0.106 vs. 0.131) -- consistent with the pattern found throughout this document on real data (fixing R200 sharpens precision, it doesn't remove a richness-driven bias). Re-sweeping `fbr` in informed mode found the same zero-bias point as blind mode, `fbr≈0.48`, and -- notably -- scatter stayed essentially flat (sd=0.106) across the entire `fbr` range tested (0.42-0.52), confirming `fbr` acts as a bias correction (an overall scaling), not a scatter reduction, exactly as understood elsewhere in this document. With `fbr` correctly calibrated for this richness in both modes, informed mode's advantage over blind mode is roughly 15-20% less scatter, holding up against true mass just as it does against measured references.

### Extending to poorer, lower-mass halos: mass vs. richness finally disentangled

The 43-halo TNG300 sample above only covers logM200=14.48-15.16 -- the massive end, matching what was originally shared. A second extraction covering the full halo mass function down to logM200=12.5 (11,942 groups total, median richness ranging from ≈40 members at the low-mass end to ≈6100 at the high-mass end) allows a direct test that wasn't possible before: since mass and richness are naturally correlated in a real halo population (more massive halos have more member subhalos), this sample can finally separate which one actually drives the `fbr` relationship, rather than relying on real-data mass terciles where the two were always confounded.

**A data-processing bug was found and fixed first**: the low-mass extraction's member positions (`dx_kpc`/`dy_kpc`/`dz_kpc`, relative to the BCG) were not corrected for periodic boundary wrapping -- TNG300's box is 205,000 ckpc/h (≈285.9 Mpc physical at this redshift), and raw relative positions before applying the minimum-image convention showed projected separations up to 403.8 Mpc and line-of-sight velocities up to ≈21,000 km/s, both wildly unphysical for a single cluster's candidates. Applying the standard minimum-image correction (`dx = dx - (|dx| > 0.5·L)·sign(dx − 0.5·L)·L` for each coordinate) fixed this immediately (dproj dropped to a sensible 0-10 Mpc range, vlos to ±4000 km/s). Anyone doing similar box-relative extractions from a periodic simulation should confirm this correction was applied before using the data.
...
**Update**: the first of these was since attempted -- a genuine ground-truth test initially against 43 massive halos from the TNG300 cosmological simulation, later extended down to logM200=12.5 (11,942 groups total; see "Validation against a cosmological simulation" above). It substantially revised, and then fully resolved, two earlier conclusions. First, the `mirror`/dynamical-state connection doesn't reliably replicate against true substructure and turns out to be viewing-angle-dependent -- genuinely unresolved, not just revised. Second, the mass-`fbr` trend: the initial 43-halo test made it look like the trend mostly disappears against true mass, but that test happened to hold richness fixed across the mass range; extending to poorer, naturally low-richness/low-mass halos showed the original richness-driven pattern cleanly (`fbr` from ≈0.42 at rich, full-sample richness up to ≈0.60-0.61 at N≈60-100) -- resolving the ambiguity completely: `fbr` tracks richness, and appeared mass-dependent on real data only because richness and mass are correlated there. Beyond revising these two, the simulation work surfaced an entirely new, arguably more important finding that wasn't on this list at all: line-of-sight projection angle alone contributes as much scatter as everything else in this document combined. That result reframes how much further precision gain should even be expected from continued parameter tuning -- a real, informative answer, if a humbling one.
