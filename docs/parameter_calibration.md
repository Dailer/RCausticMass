...
**If extending this work further**, the highest-value next steps based on everything above would be: a genuine mock-catalog test with known true membership and mass (as Serra & Diaferio 2013 did for the original method, never attempted here); a larger-sample, better-powered version of the mass-`fbr` investigation (the ~20-25 clusters per tercile used here is enough to see a real effect but not enough to build a trustworthy correction curve); and a closer look at the 2D density map shape itself (not just the 1D extracted curve) for the unresolved "falling near centre" cases, which the 1D diagnostics used throughout this document weren't able to explain.
